...
def get_today_match():
    ...
    for match in res.get("matches", []):
        if datetime.fromisoformat(match["utcDate"].replace("Z", "+00:00")).date() == today:
            return match
    return None
...
def post_lineup(match, state):
    lineup_data = match.get("match", {}).get("lineups", [])
...
def post_goals(match, state):
    goals = [e for e in match["match"].get("events", []) if e["type"] == "GOAL"]
    for goal in goals:
...
def post_subs(match, state):
    subs = [e for e in match["match"].get("events", []) if e["type"] == "SUBSTITUTION"]
    for sub in subs:
...
def post_final_score(match, state):
    if not state["final_score_posted"] and match["match"]["status"] == "FINISHED":
        score = match["match"]["score"]["fullTime"]
